...
jGrid.prototype.init = function (isSingle) {
    ...
    var jhtmlSingle = '<div id="' + this.jgridBase + '"><div class="card-style">' +
        ...
        '               <div class="j-grid-detail-caption" id="DetailsCaption" style="background-color: #ededed;border-top-left-radius: 4px;border-top-right-radius: 4px;">' + this.gridCaption + ' <div class="dropdown pull-right"><span>Export </span><img class="dropbtn" style="width:7px;" src="../Images/arrow-down.png"><div class="dropdown-content" id="ExportDiv' + this.jgridBase +'"></div></div></div>' +
        ...
        '                        </div>' +
        '                   </div>' +
        '                    <div style="display:inline-block;width:34%;text-align:right;vertical-align: top;padding-top: 6px;font-weight: bold;font-size: 12px">' +
        '                        Total Record : <span id="TotalRecord"></span>' +
        '                    </div>' +
        '               </div>' +
...
jGrid.prototype.bindChart = function (modelName,data,chartType,chartTitle) {
    
    var parm = { "ModelName": modelName, "RequestType": "Chart", "Data": "", "RecordId": data };
    var op = this;
    appetizer.actionCall(this.actionURL, parm, "GET", function (data, result) {
        if (result) {
            //console.log(data.Data);
            GraphManager.loadChartInfo(modelName, chartType, chartTitle, data.Data.Series, data.Data.Categories);
        }
        else {
            appetizer.ajaxLoder.hidePleaseWait();
            appetizer.message.showError('#showErrorMsg', data.Data, 10000);
