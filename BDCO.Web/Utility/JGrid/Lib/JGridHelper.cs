...
namespace BDCO.Web.Utility.JGrid
{
    public class JGridHelper<T> where T : class
    {
        ...
        public string ExportCSV(string sqlQuery = "")
        {
        ...
        public DataTable ExportExcel (string sqlQuery = "")
        {
            var dt = unitOfWork.GetDataTable(sqlQuery);
            //StringBuilder sb = new StringBuilder();
            //IEnumerable<string> columnNames = dt.Columns.Cast<DataColumn>().Select(column => column.ColumnName);
            ...
            if (columns.Contains("RecordId"))
            {
                dt.Columns.Remove("RecordId");
            }
            if (columns.Contains("TotalRecord"))
            {
        ...
        //public void InsertVerificationLog(string tableName, int operationType, string primaryKey, long id, string uUID, int notifiedTo, int notifiedBy, string RecordType,string childTables)
        //{
